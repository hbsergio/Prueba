...
namespace TicketO
{
    public class Ticket
    {
        ...
        {
            get { return footImage;  }
            set { if (footImage != value) footImage = value; }
        }
        ...
        public void AddImg(Image Uno) {
            headerImage = Uno;
        }

        public void ftIma(Image Dos)
        {
            footImage = Dos;
        }
        ...
        private void DrawImage()
        {
            if (headerImage != null)
            {
                try
                {
                    gfx.DrawImage(headerImage, new Point((int)leftMargin+10, (int)YPosition()));
        ...
        private void DrawFin()
        {
            if (footImage != null)
            {
                try
                {
                    gfx.DrawImage(footImage, new Point((int)leftMargin+20, (int)YPosition()));
                    double height = ((double)footImage.Height / 58) * 15;
                    imageHeight = (int)Math.Round(height) + 3;
                }
                catch (Exception)
                {
